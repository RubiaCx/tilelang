    ...
    def kernel_impl_kernel(K_desc: T.handle("uint8x128", "grid_constant"), Output_desc: T.handle("uint8x128", "grid_constant"), Q_desc: T.handle("uint8x128", "grid_constant"), V_desc: T.handle("uint8x128", "grid_constant")):
        ...
        if 128 <= tx:
            ...
            for k in range(4):
                ...
                T.fence_proxy_async()
                T.mbarrier_wait_parity(T.get_mbarrier(k % 2), k // 2)
                T.tl_gemm("tl::gemm_ss<64, 64, 64, 4, 1, 0, 1, 0, 64, 64, 0, 0, true>", T.tvm_access_ptr(T.type_annotation("float16"), buf_dyn_shmem, 16384, 4096, 1), T.tvm_access_ptr(T.type_annotation("float16"), buf_dyn_shmem, k % 2 * 4096, 4096, 1), T.tvm_access_ptr(T.type_annotation("float32"), acc_s, 0, 4096, 3))
                T.ptx_arrive_barrier(T.get_mbarrier(k % 2 + 4), 0, tx == 0)
                for i in T.unroll(2):
