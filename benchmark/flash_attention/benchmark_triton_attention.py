...
(https://tridao.me/publications/flash2/flash2.pdf), extended here to also benchmark
TileLang's flash attention kernel side-by-side with Triton and Flash-Attn.

Credits: OpenAI kernel team, Tri Dao (FlashAttention), and TileLang contributors.
"""
    ...
    for mode in ["fwd"]:
        for causal in [True, False]:
            ...
            for warp_specialize in [False, True] if enable_ws else [False]:
                base_line_vals = ["triton-fp16"] + (["flash"] if HAS_FLASH else [])
                base_line_names = ["Triton [FP16]"] + (["Flash-3"] if HAS_FLASH else [])
                if mode == "fwd":
                    line_vals = base_line_vals + ["tilelang", "tilelang-ws"]
                    line_names = base_line_names + ["TileLang", "TileLang-ws"]
                    styles = [("red", "-"), ("blue", "-"), ("green", "-"), ("purple", "-"), ("black", "-")]
                else:
                    line_vals = base_line_vals
                    line_names = base_line_names
                ...
                configs.append(
                    triton.testing.Benchmark(
                        ...
                        ylabel="TFLOPS",
                        plot_name=(
                            f"fused-attention-batch{BATCH}-head{N_HEADS}-d{HEAD_DIM}-"
                            f"{mode}-causal={causal}-warp_specialize={warp_specialize}"
                        ),
                        args={
                            "H": N_HEADS,
                            "BATCH": BATCH,
...
# ---------------------------------------------------------------------------
# TileLang flash attention loaders and kernel caches
# ---------------------------------------------------------------------------

def _load_tilelang_flash_fwd():
    ...
    mod = SourceFileLoader("tl_flash_fwd_bshd", src).load_module()
    return mod.flashattn


def _load_tilelang_flash_fwd_ws():
    here = os.path.dirname(os.path.abspath(__file__))
    repo_root = os.path.dirname(os.path.dirname(here))
    src = os.path.join(
        repo_root,
    ...
    src = os.path.abspath(src)
    mod = SourceFileLoader("tl_flash_fwd_bshd_ws", src).load_module()
    return mod.flashattn
...
def _get_tilelang_kernel(batch, heads, seqlen, dim, causal, block_m=128, block_n=128):
    key = (batch, heads, seqlen, dim, bool(causal), block_m, block_n)
    if key in _TL_KERNEL_CACHE:
        return _TL_KERNEL_CACHE[key]
    flashattn = _load_tilelang_flash_fwd()
    # flashattn(batch, heads, seq_len, dim, is_causal, block_M, block_N, ...)
    kernel = flashattn(
        batch,
        heads,
        seqlen,
    ...
    _TL_KERNEL_CACHE[key] = kernel
    return kernel


def _get_tilelang_kernel_ws(batch, heads, seqlen, dim, causal, block_m=128, block_n=128):
    """Get (and cache) warp-specialized TileLang flash attention kernel for given shape."""
    key = (batch, heads, seqlen, dim, bool(causal), block_m, block_n)
    if key in _TL_KERNEL_CACHE_WS:
        return _TL_KERNEL_CACHE_WS[key]
    flashattn_ws = _load_tilelang_flash_fwd_ws()
    # flashattn_ws(batch, heads, seq_len, dim, is_causal, block_M, block_N, ...)
    kernel = flashattn_ws(
        batch,
        heads,
        seqlen,
    ...
    _TL_KERNEL_CACHE_WS[key] = kernel
    return kernel
...
def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, causal, warp_specialize, mode, provider, device=DEVICE):
    ...
    if provider == "tilelang":
        if mode != "fwd":
            return float("nan")
        # TileLang non-WS kernel uses layout [batch, seq_len, heads, dim]
        q = torch.randn((BATCH, N_CTX, H, HEAD_DIM), dtype=dtype, device=device, requires_grad=True)
        k = torch.randn((BATCH, N_CTX, H, HEAD_DIM), dtype=dtype, device=device, requires_grad=True)
        v = torch.randn((BATCH, N_CTX, H, HEAD_DIM), dtype=dtype, device=device, requires_grad=True)
        tl_kernel = _get_tilelang_kernel(BATCH, H, N_CTX, HEAD_DIM, causal)
    ...
    if provider == "tilelang-ws":
        if mode != "fwd":
            return float("nan")
        # TileLang WS variant uses the same API/layout as non-WS
        q = torch.randn((BATCH, N_CTX, H, HEAD_DIM), dtype=dtype, device=device, requires_grad=True)
        k = torch.randn((BATCH, N_CTX, H, HEAD_DIM), dtype=dtype, device=device, requires_grad=True)
        v = torch.randn((BATCH, N_CTX, H, HEAD_DIM), dtype=dtype, device=device, requires_grad=True)
        tl_kernel = _get_tilelang_kernel_ws(BATCH, H, N_CTX, HEAD_DIM, causal)
